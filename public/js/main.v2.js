...
async function fetchTestimonials() {
    console.log("fetchTestimonials started (Parallax Marquee)");
    let items = [];
    try {
    ...
    if (items.length === 0) {
        items = [
            { name: 'Brasth Ganesh', role: 'CEO', message: 'Truly surprised by such immense levels of hard work and sincerity.' },
            ...
            { name: 'David Smith', role: 'CTO', message: 'Exceptional delivery speed and quality code.' },
            { name: 'Sarah Lee', role: 'Designer', message: 'A developer with a true eye for design details.' }
        ];
    }

    const container = document.getElementById('testimonials-container');
    if (container) {
        // Create Two Rows for Visual Complexity
        const row1Data = [...items, ...items, ...items]; // Loop data
        const row2Data = [...items.reverse(), ...items, ...items]; // Reversed loop

        container.innerHTML = `
            <div class="testimonial-marquee-wrapper">
                <!-- TOP ROW (Left to Right) -->
                <div class="marquee-row" id="marquee-row-1">
                    ${row1Data.map(t => renderTestimonialCard(t)).join('')}
                </div>
                
                <!-- BOTTOM ROW (Right to Left) -->
                <div class="marquee-row" id="marquee-row-2">
                    ${row2Data.map(t => renderTestimonialCard(t)).join('')}
                </div>
            </div>
        `;
...
function renderTestimonialCard(t) {
    return `
        <div class="testimonial-card-parallax">
            <div class="t-quote-icon"><i class="fas fa-quote-left"></i></div>
...
function initTestimonialMarquee() {
    ...
    if (row1) {
        ...
        gsap.to(row1, {
            x: - (row1Width / 3), // Move 1/3 defined by loop
            duration: 40,
            ease: "none",
            repeat: -1
        });
    ...
    if (row2) {
        ...
        gsap.set(row2, { x: - (row2Width / 3) }); // Start offset
        gsap.to(row2, {
    ...
    // PARALLAX EFFECT ON MOUSE MOVE (Subtle Tilt)
    const cards = document.querySelectorAll('.testimonial-card-parallax');
    document.addEventListener('mousemove', (e) => {
        const xPct = (e.clientX / window.innerWidth) - 0.5;
        const yPct = (e.clientY / window.innerHeight) - 0.5;

        // Animate visible cards only ideally, but all is fine for now
        gsap.to(cards, {
            x: xPct * 30, // Slight parallax movement opposite to mouse
            y: yPct * 30,
            rotationY: xPct * 10,
            rotationX: -yPct * 10,
            duration: 1.5,
